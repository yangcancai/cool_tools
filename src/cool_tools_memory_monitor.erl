...
-module(cool_tools_memory_monitor).
...
-export([init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2,
         code_change/3]).
-export([get_total_memory/0, get_vm_limit/0, get_check_interval/0, set_check_interval/1,
         get_vm_memory_high_watermark/0, set_vm_memory_high_watermark/1, get_memory_limit/0,
         get_memory_use/1, get_process_memory/0, get_process_memory/1,
         get_memory_calculation_strategy/0, get_rss_memory/0, interpret_limit/2]).
...
-type vm_memory_high_watermark() ::
    float() | {absolute, integer() | string()} | {relative, float() | integer() | string()}.
...
-spec get_cached_process_memory_and_limit() -> {non_neg_integer(), float() | infinity}.
...
set_vm_memory_high_watermark(Fraction) ->
    gen_server:call(?MODULE, {set_vm_memory_high_watermark, Fraction}, infinity).
...
get_memory_calculation_strategy() ->
    case application:get_env(cool_tools, vm_memory_calculation_strategy, rss) of
        allocated ->
            allocated;
        erlang ->
            erlang;
        legacy ->
            erlang; %% backwards compatibility
        rss ->
            rss;
        UnsupportedValue ->
            ?LOG_WARNING("Unsupported value '~tp' for vm_memory_calculation_strategy. "
...
start_link(MemFraction) ->
    start_link(MemFraction, fun alarm_handler:set_alarm/1, fun alarm_handler:clear_alarm/1).
...
init([MemFraction, AlarmFuns]) ->
    ...
    State0 =
        #state{timeout = ?DEFAULT_MEMORY_CHECK_INTERVAL,
               timer = TRef,
               alarmed = false,
...
handle_call(get_vm_memory_high_watermark,
            _From,
...
handle_call({set_check_interval, Timeout}, _From, State) ->
    State1 =
        case erlang:cancel_timer(State#state.timer) of
            false ->
                State#state{timeout = Timeout};
            _ ->
                State#state{timeout = Timeout, timer = erlang:send_after(Timeout, self(), update)}
        end,
    {reply, ok, State1};
...
get_process_memory_using_strategy(rss,
    ...
    {ok, ProcMem};
get_process_memory_using_strategy(rss, #state{os_type = {unix, _}, os_pid = OsPid}) ->
...
get_total_memory_from_os() ->
    try
        ...
        _:Error:Stacktrace ->
            ?LOG_WARNING("Failed to get total system memory: ~n~tp~n~tp", [Error, Stacktrace]),
            unknown
    end.
...
set_mem_limits(State, MemLimit) ->
    case erlang:system_info(wordsize) of
        4 ->
            ?LOG_WARNING("You are using a 32-bit version of Erlang: you may run into "
                         "memory address~n"
    ...
    TotalMemory =
        case get_total_memory() of
            unknown ->
                case State of
                    #state{total_memory = undefined, memory_limit = undefined} ->
                        ?LOG_WARNING("Unknown total memory size for your OS ~tp. "
    ...
    UsableMemory =
        case get_vm_limit() of
            Limit when Limit < TotalMemory ->
                ?LOG_WARNING("Only ~tp MiB (~tp bytes) of ~tp MiB (~tp bytes) memory usable due to "
    ...
    ?LOG_INFO("Memory high watermark set to ~tp MiB (~tp bytes)"
              " of ~tp MiB (~tp bytes) total",
              [trunc(MemLim / ?ONE_MiB), MemLim, trunc(TotalMemory / ?ONE_MiB), TotalMemory]),
...
parse_mem_limit({absolute, Limit}) ->
    case parse_information_unit(Limit) of
        {ok, ParsedLimit} ->
            {absolute, ParsedLimit};
        {error, parse_error} ->
...
parse_mem_limit(MemLimit)
    when is_float(MemLimit), MemLimit =< ?MAX_VM_MEMORY_HIGH_WATERMARK ->
    MemLimit;
parse_mem_limit(MemLimit)
    when is_float(MemLimit), MemLimit > ?MAX_VM_MEMORY_HIGH_WATERMARK ->
    ?LOG_WARNING("Memory high watermark of ~tp is above the allowed maximum, falling back to ~tp",
                 [MemLimit, ?MAX_VM_MEMORY_HIGH_WATERMARK]),
    ?MAX_VM_MEMORY_HIGH_WATERMARK;
parse_mem_limit(MemLimit) ->
    ?LOG_WARNING("Memory high watermark of ~tp is invalid, defaulting to ~tp",
                 [MemLimit, ?DEFAULT_VM_MEMORY_HIGH_WATERMARK]),
    ?DEFAULT_VM_MEMORY_HIGH_WATERMARK.

internal_update(State0 =
                    #state{memory_limit = MemLimit,
                           alarmed = Alarmed,
    ...
    case {Alarmed, NewAlarmed} of
        {false, true} ->
            emit_update_info(set, ProcMem, MemLimit),
            AlarmSet({{resource_limit, memory, node()}, []});
        {true, false} ->
            emit_update_info(clear, ProcMem, MemLimit),
...
emit_update_info(AlarmState, MemUsed, MemLimit) ->
    ?LOG_INFO("vm_memory_high_watermark ~tp. Memory used:~tp allowed:~tp",
...
get_vm_limit({win32, _OSname}) ->
    case erlang:system_info(wordsize) of
        4 ->
            2 * 1024 * 1024 * 1024;          %% 2 GB for 32 bits  2^31
        8 ->
            8 * 1024 * 1024 * 1024 * 1024      %% 8 TB for 64 bits  2^42
    end;
...
get_vm_limit(_OsType) ->
    case erlang:system_info(wordsize) of
        4 ->
            2 * 1024 * 1024 * 1024;          %% 2 GB for 32 bits  2^31
        8 ->
            256 * 1024 * 1024 * 1024 * 1024    %% 256 TB for 64 bits 2^48
...
default_linux_pagesize(CmdOutput) ->
    ?LOG_WARNING("Failed to get memory page size, using 4096. Reason: ~ts", [CmdOutput]),
    4096.
...
get_total_memory({win32, _OSname}) ->
    ...
    {ok, [_MemLoad, TotPhys, _AvailPhys, _TotPage, _AvailPage, _TotV, _AvailV], _RestStr} =
...
get_total_memory({unix, linux}) ->
    ...
    Dict =
        dict:from_list(
            lists:map(fun parse_line_linux/1, Lines)),
...
get_total_memory({unix, sunos}) ->
    ...
    Dict =
        dict:from_list(
            lists:map(fun parse_line_sunos/1, Lines)),
...
get_total_memory({unix, aix}) ->
    ...
    Dict =
        dict:from_list(
            lists:map(fun parse_line_aix/1, Lines)),
...
read_proc_file(IoDevice, Acc) ->
    case file:read(IoDevice, ?BUFFER_SIZE) of
        {ok, Res} ->
            read_proc_file(IoDevice, [Res | Acc]);
        eof ->
            Acc
    end.
...
parse_information_unit(Value) when is_integer(Value) ->
    {ok, Value};
parse_information_unit(Value0) ->
    Value = cool_tools:to_list(Value0),
    case re:run(Value,
                ...
                case Unit of
                    ...
                    GiB
                        when GiB =:= "g";
                             ...
                             GiB =:= "gib" ->
                        1024 * 1024 * 1024;
                    KB when KB =:= "KB"; KB =:= "kB"; KB =:= "kb"; KB =:= "Kb" ->
                        1000;
                    MB when MB =:= "MB"; MB =:= "mB"; MB =:= "mb"; MB =:= "Mb" ->
                        1000000;
                    GB when GB =:= "GB"; GB =:= "gB"; GB =:= "gb"; GB =:= "Gb" ->
                        1000000000
                end,
...
parse_mem_friendly(Bytes) ->
    [{_, Res}] = process_mem({mem, Bytes}, []),
    Res.

process_mem({Type, Bytes}, Acc) when Bytes < 1024 ->
    [{Type, <<(cool_tools:to_binary(Bytes))/binary, "B">>} | Acc];
process_mem({Type, Bytes}, Acc) when Bytes < 1024 * 1024 ->
    [{Type, <<(cool_tools:to_binary(Bytes / 1024))/binary, "K">>} | Acc];
process_mem({Type, Bytes}, Acc) when Bytes < 1024 * 1024 * 1024 ->
