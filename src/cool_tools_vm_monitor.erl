...
start_link() ->
  start_link(#opts{queue_len_limit = 50, alarm_funs = {fun alarm_handler:set_alarm/1, fun alarm_handler:clear_alarm/1}}).

%% @doc Spawns the server and registers the local name (unique)
-spec(start_link(opts() | integer()) ->
  {ok, Pid :: pid()} | ignore | {error, Reason :: term()}).
start_link(QueueLenLimit) when is_integer(QueueLenLimit) ->
  start_link(#opts{queue_len_limit = QueueLenLimit, alarm_funs = {fun alarm_handler:set_alarm/1, fun alarm_handler:clear_alarm/1}});
...
init([Opts]) ->
  ...
  {ok, #state{opts = Opts, timer = TRef, timeout = ?DEFAULT_QUEUE_LEN_CHECK_INTERVAL}}.
...
handle_call({set_opts, #opts{queue_len_limit = Limit, alarm_funs = {SetAlarm, ClearAlarm}} = Opts}, _, State) when Limit > 0,
  is_function(SetAlarm), is_function(ClearAlarm)->
...
%%% Internal functions
%%%===================================================================
internal_update(#state{opts = #opts{queue_len_limit = Limit, alarm_funs = {SetAlarm, ClearAlarm}}, alarmed = Alarmed} = S) ->
