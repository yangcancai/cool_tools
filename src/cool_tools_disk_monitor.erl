...
-module(cool_tools_disk_monitor).
...
-export([init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2,
         code_change/3]).
-export([get_disk_free_limit/0, set_disk_free_limit/1, get_min_check_interval/0,
         set_min_check_interval/1, get_max_check_interval/0, set_max_check_interval/1,
         get_disk_free/0, set_enabled/1, set_disk_dir/1]).
...
-type disk_free_limit() ::
    integer() | {absolute, integer()} | string() | {mem_relative, float() | integer()}.
...
set_disk_dir(Dir) ->
    gen_server:call(?MODULE, {set_dis_dir, Dir}).
...
init([Limit]) ->
    Dir = dir(),
    Retries = application:get_env(cool_tools, disk_monitor_failure_retries, 10),
    Interval = application:get_env(cool_tools, disk_monitor_failure_retry_interval, 60000),
...
handle_call({set_enabled, _Enabled = false}, _From, State = #state{enabled = false}) ->
    ...
    {reply, ok, State#state{enabled = false}};
handle_call({set_disk_dir, Dir}, _From, State) ->
    {reply, ok, State#state{dir = Dir}};
...
run_port_cmd(Cmd0, Port) ->
    ...
    Cmd2 = cool_tools:to_utf8_binary(Cmd1),
...
dir() ->
    application:get_env(cool_tools, disk_dir, "/").
...
set_disk_limits(State, Limit0) ->
    ...
    ?LOG_INFO("Disk free limit set to ~bMB", [trunc(Limit / 1000000)]),
...
get_disk_free(Dir, {win32, _}, not_used) ->
    ...
    case win32_get_drive_letter(Dir) of
        ...
        DriveLetter ->
            ...
            ["DRIVE_FIXED",
             FreeBytesAvailableToCallerStr,
             _TotalNumberOfBytesStr,
             _TotalNumberOfFreeBytesStr] =
                string:tokens(DriveInfo, " "),
    ...
    case string:tokens(Str, "\n") of
        [_, S | _] ->
            case string:tokens(S, " \t") of
                ...
                _ ->
                    exit({unparseable, Str})
            end;
        _ ->
            exit({unparseable, Str})
    end.

win32_get_drive_letter([DriveLetter, $:, $/ | _])
    when DriveLetter >= $a andalso DriveLetter =< $z ->
...
win32_get_disk_free_dir(Dir) ->
    ...
    case run_os_cmd("dir /-C /W \"" ++ RawDir ++ "\"") of
        ...
        CommandResult ->
            LastLine0 =
                lists:last(
                    string:tokens(CommandResult, "\r\n")),
            LastLine1 = lists:reverse(LastLine0),
            {match, [Free]} = re:run(LastLine1, "(\\d+)", [{capture, all_but_first, list}]),
            {ok, list_to_integer(lists:reverse(Free))}
    end.

interpret_limit({mem_relative, Relative}) when is_number(Relative) ->
    round(Relative * cool_tools_memory_monitor:get_total_memory());
interpret_limit({absolute, Absolute}) ->
    interpret_limit(Absolute);
interpret_limit(Absolute) ->
    case cool_tools_memory_monitor:parse_information_unit(Absolute) of
        {ok, ParsedAbsolute} ->
            ParsedAbsolute;
        {error, parse_error} ->
            ?LOG_ERROR("Unable to parse disk_free_limit value ~tp", [Absolute]),
            ?DEFAULT_DISK_FREE_LIMIT
    end.

emit_update_info(StateStr, CurrentFree, Limit) ->
    ?LOG_INFO("Free disk space is ~ts. Free bytes: ~b. Limit: ~b",
              [StateStr, CurrentFree, Limit]).

start_timer(State) ->
    State#state{timer = erlang:send_after(interval(State), self(), update)}.

interval(#state{alarmed = true, max_interval = MaxInterval}) ->
    MaxInterval;
...
enable_handle_disk_free(DiskFree, State) when is_integer(DiskFree) ->
    enable_handle_total_memory(catch cool_tools_memory_monitor:get_total_memory(),
                               DiskFree,
                               State);
...
enable_handle_total_memory(TotalMemory, DiskFree, #state{limit = Limit} = State)
    when is_integer(TotalMemory) ->
    ?LOG_INFO("Enabling free disk space monitoring "
              "(disk free space: ~b, total memory: ~b)",
              [DiskFree, TotalMemory]),
    start_timer(set_disk_limits(State, Limit));
enable_handle_total_memory(Error,
                           _DiskFree,
                           #state{interval = Interval, retries = Retries} = State) ->
    ?LOG_WARNING("Free disk space monitor encountered an error "
...
run_os_cmd(Cmd) ->
    ...
    CmdFun =
        fun() ->
           CmdResult = cool_tools:os_cmd(Cmd),
