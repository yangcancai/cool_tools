...
%%%
%%% @end
%%% Created : 2021-09-08T09:23:14+00:00
%%%-------------------------------------------------------------------
-module(cool_tools_compile).
...
from_string(CodeStr, CompileFormsOptions) ->
    ...
    case compile:forms(Forms, CompileFormsOptions) of
        ...
        {ok, ModuleName, CompiledCodeBinary, []}
            when is_binary(CompiledCodeBinary) ->  % empty warnings list
            {ModuleName, CompiledCodeBinary};
...
%% @private
-spec pre_proc([{_, _} | {_, _, _}], macro_dict()) ->
                  {include, _} | {macro, macro_dict()} | {tokens, [any()]}.
...
%% @private
-spec macro_params_body_def(Tokens :: [{_, _} | {_, _, _}, ...], RevParams :: [any()]) ->
                               {[any()], [{_, _} | {_, _, _}]}.
...
%% @private
-spec subst_macros_get_args(Toks :: nonempty_maybe_improper_list(), RevArgs :: [any()]) ->
                               {_, [any()]}.
